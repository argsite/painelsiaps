...
def montar_motivo_pendencia_siaps_hipertensao(row):
    motivos = []
    if row.get('pendencia_A', False): motivos.append('Sem consulta em 6 meses')
    if row.get('pendencia_B', False): motivos.append('Sem aferição de PA em 6 meses')
    if row.get('pendencia_C', False): motivos.append('Sem peso e altura em 12 meses')
    if row.get('pendencia_D', False): motivos.append('Sem 2 visitas ACS/TACS em 12 meses')
...
def preparar_siaps_hipertensao_boas_praticas(df):
    ...
    df['total_boas_praticas'] = df['cumpriu_A'].astype(int)+df['cumpriu_B'].astype(int)+df['cumpriu_C'].astype(int)+df['cumpriu_D'].astype(int)
    df['total_pendencias'] = df['pendencia_A'].astype(int)+df['pendencia_B'].astype(int)+df['pendencia_C'].astype(int)+df['pendencia_D'].astype(int)
...
def calcular_metricas_siaps_hipertensao(df):
    ...
    return dict(total_pacientes=total_pacientes, denominador=denominador, numerador=numerador, desempenho=desempenho, cumpriu_a=cumpriu_a, cumpriu_b=cumpriu_b, cumpriu_c=cumpriu_c, cumpriu_d=cumpriu_d, pend_a=pend_a, pend_b=pend_b, pend_c=pend_c, pend_d=pend_d, boas_praticas_cumpridas=boas_praticas_cumpridas, boas_praticas_possiveis=boas_praticas_possiveis, cobertura_boas_praticas=cobertura_boas_praticas)
...
def render_siaps_hipertensao_boas_praticas(df):
    ...
    exibirmetricascards(('Total de pacientes', m['total_pacientes']), ('Denominador', m['denominador']), ('Numerador', m['numerador']), ('Desempenho', f"{m['desempenho']:.1f}%"), ('Boas práticas', f"{m['boas_praticas_cumpridas']}/{m['boas_praticas_possiveis']}"))
    exibirmetricascards(('Pendência A', m['pend_a']), ('Pendência B', m['pend_b']), ('Pendência C', m['pend_c']), ('Pendência D', m['pend_d']))
    cobertura = pd.DataFrame({'Boa prática':['A - Consulta','B - PA','C - Peso/altura','D - Visitas ACS'],'Cumpridos':[m['cumpriu_a'],m['cumpriu_b'],m['cumpriu_c'],m['cumpriu_d']],'Pendentes':[m['pend_a'],m['pend_b'],m['pend_c'],m['pend_d']]})
    ...
    somente_pendentes = st.checkbox('Mostrar apenas pacientes com alguma pendência', value=True, key='siaps_hipertensao_somente_pendentes')
    lista = df.copy()
    ...
    colunas_saida = ['CPF', 'CNS', 'Nascimento', 'Sexo', 'Raça cor', 'Raça/Cor descrita', 'CNES', 'INE', 'A', 'B', 'C', 'D', 'NM', 'DN', 'total_boas_praticas', 'total_pendencias', 'Motivo da pendência']
    colunas_saida = [c for c in colunas_saida if c in lista.columns]
    st.download_button('Exportar lista nominal SIAPS - Hipertensão', data=dataframeparaexcelbytes(lista[colunas_saida]), file_name='siaps_hipertensao_boas_praticas.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', key='download_siaps_hipertensao')
    ...
    try:
        ...
        if tipo_siaps == 'SIAPS_HIPERTENSAO_BOAS_PRATICAS':
            ...
            if linhadetectada is None:
                ...
                trocar = st.checkbox('Trocar linha de cuidado manualmente', key='trocarlinhacuidado')
                if trocar:
                    secao = st.radio('Linha de cuidado', ['Hipertensão', 'Diabetes'], index=0 if secao == 'Hipertensão' else 1, horizontal=True, key='linhadetectadaeditavel')
                    origem = 'manual'
            exibir_cabecalho_analise(secao, origem)
            st.success('Fluxo antigo carregado com sucesso.')
