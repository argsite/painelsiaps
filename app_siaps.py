...
if arq_siaps and arq_cad:
    ...
    # Criamos as colunas de presença antes do merge
    ...
    # FILTRO FINAL: Apenas linhas que possuem CPF válido de 11 dígitos
    df_final = df_final[df_final['CPF_key'].str.match(r'^\d{11}$')]
    df_final = df_final[~df_final['CPF_key'].isin(['00000000000', ''])]
    ...
    indicadores = {'A': 'A - Consultas', 'B': 'B - P.A.', 'C': 'C - Peso/Altura', 'D': 'D - Visitas'}
    for i, (key, label) in enumerate(indicadores.items()):
    ...
    dados = [{'Boas Práticas': label, 'Total': df_lista[key].astype(str).str.strip().str.upper().value_counts().get('X', 0)} 
             for key, label in indicadores.items()]
    ...
    st.info('Por favor, envie ambas as planilhas.')
